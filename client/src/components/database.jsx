...
export const Database = () => {
    ...
    const generate = (arr) => {
        return arr?.map((i) => {
            let { user_id, chat_type, chat_id, content } = i
            return {
                user_id: user_id,
                chat_type: chat_type,
                chat_id: chat_id,
    ...
    return (
        <div className='h-[800px] w-full bg-white rounded-2xl py-2 relative'>
            ...
            <p className='text-sm opacity-50'>infromation fetched from elephant sql</p>
            {view}
            {!data && <button onClick={fetchDB} className='text-white'> request</button>}
            {data &&
                <>
                    <p className='text-sm font-medium mb-2'>users loaded: {database?.length}</p>
                    <CSVLink data={data} headers={csv} className='  h-[32px] text-sm border border-blue-100 px-4 py-2 mt-2 rounded-md'>
                        Download as CSV
                    </CSVLink>
                </>
            }
